...
const getGradientDirection = (position: string) =>
  (({ top: 'to top', bottom: 'to bottom', left: 'to left', right: 'to right' } as any)[position] || 'to bottom');
...
function GradualBlur(props: any) {
  ...
  return (
    <div
      ref={containerRef}
      className={`gradual-blur ${config.target === 'page' ? 'gradual-blur-fixed' : ''} ${config.className}`}
